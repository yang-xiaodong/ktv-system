  ...
  methods: {
    getPackage() {
      return new Promise((resolve, reject) => {
        this.$http.post(apiUrl.getPackage).then(res => {
    ...
    getRoomType() {
      return new Promise((resolve, reject) => {
        this.$http.post(apiUrl.getRoomType).then(res => {
          if (res.data.code === 200) {
            resolve(res.data.message);
          } else {
            reject();
    ...
    async edit({ type, params = {} }) {
      if (!this.hasGet) {
        ...
        this.hasGet = true;
        this.tableOptions.formArray.splice(0, 1, {
          key: "roomType",
          title: "类型",
          type: "select",
          options: this.roomType
        });
        this.tableOptions.formArray.splice(3, 1, {
          key: "package",
          title: "套餐",
          type: "checkbox",
          tooltip: true,
