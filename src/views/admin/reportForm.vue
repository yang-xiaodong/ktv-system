  ...
  <div class="report-container">
    <div class="condition-wrapper">
      <Button
        ...
        @click="filterReportData(item.key, i)"
        :key="i"
      >{{ item.title }}</Button>
      <DatePicker
        type="daterange"
  ...
  methods: {
    ...
    filterReportData(key, index) {
      ...
      switch (key) {
        ...
        case "year":
          timeRange = [
            today - 3600 * 1000 * 24 * 360,
            today + 3600 * 1000 * 24
          ];
          break;
        default:
          timeRange = [
            new Date(key[0]).getTime() - 3600 * 1000 * 8,
            new Date(key[1]).getTime() + 3600 * 1000 * 16
          ];
          break;
      }
      if (key === "all") {
    ...
    formatGoodsData() {
      ...
      this.orderList.forEach(ele => {
        ...
        for (let key in goods) {
          let item = goods[key];
          qtyMap[key] =
            qtyMap[key] === undefined
              ? Number(item.qty)
              : Number(qtyMap[key]) + Number(item.qty);
        }
        ...
        goodsArray.forEach(item => {
          if (grp.indexOf(item.goods + "") === -1) {
            qtyMap[item.goods] =
              qtyMap[item.goods] === undefined
                ? Number(item.qty)
                : Number(qtyMap[item.goods]) + Number(item.qty);
          }
        });
        ...
        stockGoods.forEach(item => {
          qtyMap[item.goods] =
            qtyMap[item.goods] === undefined
              ...
              : Number(qtyMap[item.goods]) + Number(item.stockQty);
        });
        depositGoods.forEach(item => {
          qtyMap[item.goods] =
            qtyMap[item.goods] === undefined
              ? Number(item.depositQty)
              : Number(qtyMap[item.goods]) - Number(item.depositQty);
        });
      });
