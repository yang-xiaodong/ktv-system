<template>
  <div class="bill-container" :class="mobileFlag ? 'mobileFlag-bill-container' : ''">
    ...
    <div class="bill-wrapper">
      <div v-if="mobileFlag">
        <div class="mobile-back" @click="hideBill">
          <icon-font icon="icon-back" fontSize="22"></icon-font>
        </div>
      ...
      <ul class="ord-info">
        <li v-for="(ord, i) in ordList" :key="i">
          <span>{{ ord.title }}：</span>
          <span
            v-if="ord.key === 'vip' && ordInfo.room && !ordInfo[ord.key] && !mobileFlag"
            class="order-vip"
            @click="loginVip"
          >登陆</span>
      ...
      <div class="account-wrapper">
        <div class="account-item" v-for="(item, i) in accountList" :key="i">
          ...
          <span :class="item.class" v-if="item.key == 'discount'">
            <input
          ...
          <span :class="item.class" v-if="item.key == 'paymethos'" @click="openPayMethod">
            ...
            <icon-font
              v-show="payMethodMap[ordInfo.payMethod]"
              :icon="payMethodMap[ordInfo.payMethod] && payMethodMap[ordInfo.payMethod].icon"
              fontSize="12"
            ></icon-font>
          </span>
      ...
      <div class="button-wrapper" v-if="!mobileFlag">
        <Button
          type="primary"
          :disabled="ordInfo.nun !== undefined"
          @click="placeOrder"
        >{{ ordInfo.nun !== undefined ? '已开单' : '开单' }}</Button>
    ...
    <Modal v-model="modalVisible" :footer-hide="true" title="选择支付方式">
      <div class="payment-wrapper">
        <div
          class="payment-item"
          v-for="(item, i) in payMethodMap"
          :key="i"
          @click="selectPayMethod(item, i)"
        >
      ...
      <div class="payment-group" v-show="paymentGroupFlag">
        <div class="payment-group-item">
          <div>
            <span>支付宝：</span>
            <InputNumber :min="0" v-model="payMoney[0]"></InputNumber>
          </div>
          <div>
            <span>微信：</span>
            <InputNumber :min="0" v-model="payMoney[1]"></InputNumber>
          </div>
          <div>
            <span>现金：</span>
            <InputNumber :min="0" v-model="payMoney[2]"></InputNumber>
          </div>
        </div>
        <div class="payment-group-item">
          <div>
            <span>刷卡：</span>
            <InputNumber :min="0" v-model="payMoney[3]"></InputNumber>
          </div>
          <div>
            <span>余额：</span>
            <InputNumber :min="0" v-model="payMoney[4]"></InputNumber>
          </div>
        </div>
    ...
    <Modal v-model="modalVip" title="会员登陆" :footer-hide="true">
      <Form
        ref="formVip"
        :model="vipFormData"
        :rules="ruleValidate"
      ...
      >
        ...
        <FormItem prop="verifyCode" label="验证码">
          <i-input v-model="vipFormData.verifyCode">
            ...
            >{{ verifyCodeBtn + (typeof verifyCodeBtn === 'number' ? ' s' : '') }}</Button>
          </i-input>
        </FormItem>
      </Form>
    ...
    <Modal v-model="modalRecharge" title="充值" :footer-hide="true">
    ...
    <Modal v-model="printFlag" title="打印账单" fullscreen>
    ...
    <Modal v-model="depositFlag" title="会员商品寄取" :footer-hide="true">
      ...
      <Table
        ...
        border
        size="small"
        @on-selection-change="depositTakeSelect"
      ></Table>
      <div class="modal-button">
...
export default {
        ...
        { key: "qty", title: "数量" },
        {
          ...
          render: (h, params) => {
            ...
            return h("InputNumber", {
              props: {
                min: 0,
                max: Number(params.row.qty),
                value: params.row.depositQty
              },
              on: {
                input(val) {
          ...
          render: (h, params) => {
            ...
            return h("InputNumber", {
              ...
              on: {
                input(val) {
                  self.$set(
                    self.takeGoodsData[params.index],
                    "depositQty",
                    val
                  );
  ...
  computed: {
    totalPrice() {
      let price = 0;
      const packagePrice =
        this.packageSelected.type == "1"
          ? this.packageSelected.price1
          : this.packageSelected.price2;
      for (let key in this.goodsSelected) {
        price +=
          Number(this.goodsSelected[key].price) *
          Number(this.goodsSelected[key].qty);
      }
  ...
  methods: {
    ...
    okVip() {
      this.$http
        .post(apiUrl.loginVip, {
          data: {
            phone: this.vipFormData.phone,
            verifyCode: this.vipFormData.verifyCode
          }
        })
        .then(res => {
          if (res.data.code === 200) {
            ...
            this.setOrdInfo({
              data: { vip: vipInfo.phone, balance: vipInfo.balance }
            });
            this.modalVip = false;
            this.updateOrder();
          } else {
    ...
    okRecharge() {
      ...
      if (
        this.rechargeForm.rechargeMoney == "" ||
        this.rechargeForm.rechargeMoney == "0" ||
        !/^\d*$/.test(this.rechargeForm.rechargeMoney) ||
        (this.rechargeForm.giveMoney != "" &&
          !/^\d*$/.test(this.rechargeForm.rechargeMoney))
      ) {
      ...
      this.$http
        .post(apiUrl.recharge, {
          data: {
            phone: this.ordInfo.vip,
            rechargeMoney: this.rechargeForm.rechargeMoney,
            giveMoney: this.rechargeForm.giveMoney
          }
        })
        .then(res => {
          if (res.data.code === 200) {
            ...
            this.setOrdInfo({
              data: {
                vip: this.ordInfo.vip,
                balance:
                  Number(this.row.balance) +
                  Number(this.rechargeForm.rechargeMoney) +
                  Number(this.rechargeForm.giveMoney)
              }
            });
    ...
    closeOrder() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.printOrdInfo = deepClone(this.ordInfo);
          this.$http
            .post(apiUrl.closeOrder, {
              data: { ordInfo: this.ordInfo }
            })
            .then(res => {
              this.setOrdInfo({
                data: { room: this.ordInfo.room },
                type: "ordInfo",
                roomSelected: "close"
              });
    ...
    openPayMethod() {
      ...
      this.paymentGroupFlag =
        sum == this.totalPrice - this.discountMoney ? true : false;
      ...
      if (this.depositTakeTab === "deposit") {
        ...
        this.$http
          .post(apiUrl.deposit, {
            data: {
              vip: this.ordInfo.vip,
              nun: this.ordInfo.nun,
              goodsList: this.depositList
            }
          })
          .then(res => {
            if (res.data.code === 200) {
              ...
              this.setOrdInfo({
                data: deepClone(this.depositList),
                type: "depositGoods"
              });
              this.updateOrder();
            }
  ...
  watch: {
    roomSelected() {
      ...
      if (this.ordInfo && this.ordInfo.payMethod !== undefined) {
        ...
        this.payMoney =
          this.ordInfo.payMoney.length === 0
            ? [0, 0, 0, 0, 0]
            : this.ordInfo.payMoney;
