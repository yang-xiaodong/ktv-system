<template>
  <div class="room-detail">
    <div class="left-wrapper">
      <ul class="title-wrapper">
        <li class="title-item" v-for="(item, i) in titleArray" :key="i">
          ...
          <span
            v-if="item.field !== 'startTime'"
            class="title-content"
          >{{ roomInfo[item.field] | status({field: item.field, roomInfo: roomInfo, ordInfo: ordInfo}) }} {{ item.after }}</span>
          <span
            v-if="item.field === 'startTime' && ordInfo.startTime"
            class="title-content"
          >{{ ordInfo.startTime | filterTime(true)}}{{ currentTime | filterTime(false)}}， {{ diffTime }}</span>
        </li>
      </ul>
    ...
    <Modal v-model="modalFlag" :title="modalTitle" :footer-hide="true">
      <CheckboxGroup v-show="goodsList.length !== 0" v-model="goodsListSelected" class="goods-list">
        <Checkbox
          ...
          disabled
          :key="i"
        >{{ item.goodsm }}({{ item.qty }}{{ item.unitm }})</Checkbox>
      </CheckboxGroup>
      <RadioGroup v-show="goodsListGrp.length !== 0" v-model="packageItem.grpSelected">
        <Radio
          class="goods-list-item"
          v-for="(item, i) in goodsListGrp"
          :label="item.goods"
          :key="i"
        >{{ item.goodsm }}({{ item.qty }}{{ item.unitm }})</Radio>
      </RadioGroup>
...
export default {
  ...
  methods: {
    clickPackage(item, index) {
      ...
      if (
        this.ordInfo.package &&
        this.ordInfo.package.type == this.packageType &&
        this.ordInfo.package.package == this.packageItem.package
      ) {
        this.$set(
          this.packageItem,
          "grpSelected",
          Number(this.ordInfo.package.grpSelected) || ""
        );
      }
    ...
    selectPackage() {
      if (
        this.packageItem.goods.length !== 0 &&
        this.packageItem.grp != "" &&
        this.packageItem.grpSelected == ""
      ) {
  ...
  filters: {
    status(value, { field, roomInfo, ordInfo }) {
      if (field === "status") {
        return roomInfo.status === undefined
          ? ""
          : roomInfo.status == 0
          ? "空闲"
          : "使用中";
