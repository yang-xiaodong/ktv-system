<template>
  <div class="goods-wrapper">
    <div class="goods-item">
      <Card
        class="card-item"
        :class="{active: goodsMap[goods.id] || goodsSelected[goods.id]}"
        v-for="(goods, i) in goodsList"
        :key="i"
        @click.native="selectGoods(goods, $event)"
      >
...
export default {
  ...
  methods: {
    getGoods() {
      this.$http.post(apiUrl.getGoods).then(res => {
  ...
  watch: {
    roomSelected() {
      ...
      this.goodsList.forEach((ele, i) => {
        ele.qty =
          (this.goodsSelected &&
            this.goodsSelected[ele.id] &&
            Number(this.goodsSelected[ele.id].qty)) ||
          1;
      });
    },
    ordInfo() {
