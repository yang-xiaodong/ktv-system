<template>
  <div class="basetable-container">
    <div class="sift-wrapper">
      ...
      <div class="order-list" v-if="mobileFlag">
        <div class="order-item" v-for="(item, i) in tableData" :key="i" @click="selectRow(item)">
          <p>
            <span class="order-item-title">包间号</span>
            {{ item.room }}
          </p>
          <p>
            <span class="order-item-title">开始时间</span>
            {{ item.startTime | filterTime }}
          </p>
          <p>
            <span class="order-item-title">结束时间</span>
            {{ item.endTime | filterTime }}
          </p>
          <p>
            <span class="order-item-title">相对时间</span>
            <Time :time="item.endTime"></Time>
          </p>
        </div>
      </div>
    ...
    <Drawer title="新增" v-model="addFlag" width="50%" :mask-closable="false" :styles="styles">
      <Form
        ref="submitForm"
        label-position="right"
        :label-width="tableOptions.width || 60"
        :model="formData"
        :rules="tableOptions.ruleValidate || {}"
      >
            ...
            >
              <Checkbox v-for="(option, j) in item.options" :label="option.id" :key="j">
                ...
                <InputNumber
                  ...
                  :min="1"
                  size="small"
                  v-model="formData['goodsQty'][option.id]"
                  style="width:50px"
                ></InputNumber>
              </Checkbox>
            </CheckboxGroup>
    ...
    <Modal v-model="goodsQtyFlag" :title="params.name" @on-ok="stockIn">
...
export default {
      ...
      let obj = {
        ...
        render: (h, params) => {
          return h("div", [
            ...
            this.tableOptions.vip || this.tableOptions.user
              ? ""
              : h("span", { class: { "operate-divide": true } }, "|"),
            this.tableOptions.vip ||
            (this.tableOptions.user && params.row.root == 1)
              ? ""
              : h(
                  "span",
                  {
  ...
  methods: {
    ...
    search() {
      if (
        this.searchPhone !== "" &&
        !/^1[34578]\d{9}$/.test(this.searchPhone)
      ) {
    ...
    getData(params = {}) {
      this.loading = true;
      this.$http
        .post(apiUrl[this.tableOptions.siftApi], {
          data: params
        })
        .then(res => {
          if (res.data.code === 200) {
    ...
    goDelete(params) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$http
            .post(apiUrl[this.tableOptions.delApi], {
              data: [params.row]
            })
            .then(res => {
              if (res.data.code === 200) {
    ...
    submitForm() {
      this.$refs.submitForm.validate(valid => {
        ...
        if (!this.updFlag) {
          this.$http
            .post(apiUrl[this.tableOptions.addApi], {
              data: [this.formData]
            })
            .then(res => {
              if (res.data.code === 200) {
          ...
          this.$http
            .post(apiUrl[this.tableOptions.updApi], {
              data: this.formData
            })
            .then(res => {
              if (res.data.code === 200) {
