...
router.post("/register", async ctx => {
  try {
    ...
    const result = await query(
      `SELECT * FROM user WHERE account = ? AND off != 1`,
      [account]
    );
    if (result.length !== 0) {
    ...
    await query(
      `INSERT INTO user (name, account, password, type, root, createTime) VALUES ( ?, ?, ?, ?, ?, ? )`,
      [name, account, password, type, type, new Date().getTime()]
    );
