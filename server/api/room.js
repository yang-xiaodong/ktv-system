  ...
  try {
    ...
    for (let i = 0, len = data.length; i < len; i++) {
      ...
      const roomtype = await query(
        `SELECT * FROM roomtype WHERE name = ? AND off != 1`,
        [item.name]
      );
      ...
      await query(`INSERT INTO roomtype (name, createTime) VALUES (?, ?)`, [
        item.name,
        currentTime
      ]);
    }
...
router.post("/getRoomType", async ctx => {
  try {
    ...
    const count = await query(
      `SELECT COUNT(*) as count FROM roomtype WHERE off != 1`
    );
    const roomTypeList = await query(
      `SELECT * FROM roomtype WHERE off != 1 ORDER BY createTime ASC`
    );
...
router.post("/deleteRoomType", async ctx => {
  try {
    ...
    await query(
      `UPDATE roomtype SET off = 1, updateTime = ? WHERE id IN ( ? )`,
      [new Date().getTime(), ids.join()]
    );
...
router.post("/updRoomType", async ctx => {
  try {
    ...
    await query(`UPDATE roomtype SET name = ?, updateTime = ? WHERE id = ?`, [
      data.name,
      new Date().getTime(),
      data.id
    ]);
    const result = await query(
      `SELECT * FROM roomtype WHERE off != 1 AND id = ?`,
      [data.id]
    );
...
router.post("/insertRoomInfo", async ctx => {
  try {
    ...
    for (let i = 0, len = data.length; i < len; i++) {
      ...
      let roomInfo;
      if (position == "") {
        roomInfo = await query(`SELECT * FROM room WHERE no = ? AND off != 1`, [
          item.no
        ]);
      } else {
        roomInfo = await query(
          `SELECT * FROM room WHERE no = ? or position = ? AND off != 1`,
          [item.no, position]
        );
      }
      ...
      for (let j = 0; j < package.length; j++) {
        await query(
          `INSERT INTO room (uuid, roomType, name, no, package, descr, position, createTime) VALUES 
          (?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            uuid,
            item.roomType,
...
router.post("/getRoomInfo", async ctx => {
  try {
    ...
    const count = await query(
      `SELECT COUNT(DISTINCT uuid) as count FROM room WHERE off != 1`
    );
    ...
    result.forEach(ele => {
      if (ele.package) {
        packageGoodsMap[ele.uuid + "_" + ele.package] =
          packageGoodsMap[ele.uuid + "_" + ele.package] || [];
        if (ele.goods) {
          packageGoodsMap[ele.uuid + "_" + ele.package].push({
            goods: ele.goods,
            goodsm: ele.goodsm,
            qty: ele.qty,
            unitm: ele.unitm
    ...
    result.forEach(ele => {
      if (roomPackageMap[ele.uuid] === undefined) {
        ...
        if (ele.package) {
          roomPackageMap[ele.uuid].value.push({
            package: ele.package,
            packagem: ele.packagem,
            grp: ele.grp,
            type1: ele.type1,
            type2: ele.type2,
            price1: ele.price1,
            price2: ele.price2,
            descr: ele.descrPackage,
            goods: packageGoodsMap[ele.uuid + "_" + ele.package]
          });
        }
      } else {
...
router.post("/updRoomInfo", async ctx => {
  try {
    ...
    for (let i = 0; i < package.length; i++) {
      await query(
        `INSERT INTO room (uuid, roomType, name, no, package, descr, position, createTime, updateTime) VALUES 
        (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          data.uuid,
          data.roomType,
