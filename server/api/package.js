...
router.post("/insertPackage", async ctx => {
  try {
    ...
    for (let i = 0, len = data.length; i < len; i++) {
      ...
      if (goods.length === 0) {
        await query(
          `INSERT INTO package (uuid, name, type1, type2, price1, price2, descr, createTime) VALUES
        (?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            uuid,
            item.name,
        ...
        for (let i = 0, len = goods.length; i < len; i++) {
          ...
          await query(
            `INSERT INTO package (uuid, name, goods, qty, grp, type1, type2, price1, price2, descr, createTime) VALUES
            (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
              uuid,
              item.name,
...
router.post("/getPackage", async ctx => {
  try {
    ...
    const count = await query(
      `SELECT COUNT(DISTINCT uuid) as count FROM package WHERE off != 1`
    );
    ...
    result.forEach(ele => {
      if (uuidMap[ele.uuid] === undefined) {
        ...
        if (ele.goods != null && ele.goods != "") {
          packageList[uuidMap[ele.uuid]].goods.push({
            id: ele.goods,
            name: ele.goodsm,
            unitm: ele.unitm,
            qty: ele.qty
...
router.post("/updPackage", async ctx => {
  try {
    ...
    if (goods.length === 0) {
      await query(
        `INSERT INTO package (uuid, name, type1, type2, price1, price2, descr, createTime, updateTime) VALUES
      (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          uuid,
          item.name,
      ...
      for (let i = 0, len = goods.length; i < len; i++) {
        const qty = item.goodsQty[goods[i]];
        await query(
          `INSERT INTO package (uuid, name, goods, qty, grp, type1, type2, price1, price2, descr, createTime, updateTime) VALUES
          (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            uuid,
            item.name,
