...
router.post("/insertOrder", async ctx => {
  try {
    ...
    await query(
      `INSERT INTO roomorder (nun, room, package, packageType, grpSelected, goods, qty, startTime, vip, totalPrice, discount, paymethod, user, createTime) VALUES 
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        nun,
        ordInfo.room,
...
router.post("/getOrder", async ctx => {
  try {
    ...
    const data = ctx.request.body.data;
    let ordInfo = await query(
      `SELECT * FROM roomorder WHERE room = ? AND close != 1 AND off != 1`,
      [data.no]
    );

    if (ordInfo.length !== 0) {
      if (ordInfo[0].vip) {
        const vipInfo = await query(
          `SELECT balance FROM vip WHERE phone = ? AND off != 1`,
          [ordInfo[0].vip]
        );
      ...
      const packageList = await query(
        `SELECT p.descr, p.name as packagem, p.uuid as package, p.type1, p.type2, p.price1, p.price2, p.qty, p.grp, g.id as goods, g.name as goodsm, u.name as unitm
        FROM package as p 
      ...
      packageMap = {
        descr: packageList[0].descr,
        package: packageList[0].package,
        packagem: packageList[0].packagem,
        type: ordInfo[0].packageType,
        grpSelected: ordInfo[0].grpSelected,
        type1: packageList[0].type1,
        type2: packageList[0].type2,
        price1: packageList[0].price1,
        price2: packageList[0].price2,
        grp: packageList[0].grp || "",
        goods: []
      };
      packageList.forEach(ele => {
        if (ele.goods) {
          packageMap.goods.push({
            goods: ele.goods,
            goodsm: ele.goodsm,
            qty: ele.qty,
            unitm: ele.unitm
      ...
      for (let i = 0, len = goodsList.length; i < len; i++) {
        let result = await query(
          `SELECT g.id, g.name, g.picture, g.price, g.descr, g.vipDiscount, g.discount, g.qty, g.record,
          u.id as unit, u.name as unitm FROM goods g, unit u WHERE u.id = g.unit AND g.id = ?`,
          [goodsList[i]]
        );
      ...
      const stockGoodsList = ordInfo[0].stockGoods
        ? ordInfo[0].stockGoods.split(",")
        : [];
      const stockQtyList = ordInfo[0].stockQty
        ? ordInfo[0].stockQty.split(",")
        : [];
      let stockGoods = [];
      for (let i = 0, len = stockGoodsList.length; i < len; i++) {
        let result = await query(
          `SELECT g.id as goods, g.name as goodsm,
          u.id as unit, u.name as unitm FROM goods g, unit u WHERE u.id = g.unit AND g.id = ?`,
          [stockGoodsList[i]]
        );
      ...
      ordInfo[0].payMoney = ordInfo[0].payMoney
        ? ordInfo[0].payMoney.split(",")
        : [];
...
router.post("/closeOrder", async ctx => {
  try {
    ...
    if (ordInfo.vip && ordInfo.payMethod == 5) {
      const vipInfo = await query(
        `SELECT * FROM vip WHERE phone = ? AND off != 1`,
        [ordInfo.vip]
      );
      await query(
        `UPDATE vip SET balance = ?, status = ? WHERE phone = ? AND off != 1`,
        [
          Number(vipInfo[0].balance) -
            Number(ordInfo.totalPrice) -
            Number(ordInfo.discount || 0),
          0,
          ordInfo.vip
        ]
      );
    }
    await query(`UPDATE roomorder SET close = 1, endTime = ? WHERE nun = ?`, [
      new Date().getTime(),
      ordInfo.nun
    ]);
    ...
    for (let i = 0, len = stockGoods.length; i < len; i++) {
      ...
      if (diffQty == 0) {
        await query(
          `DELETE FROM vipstock WHERE VIP = ? AND goods = ? AND off != 1`,
          [ordInfo.vip, goods]
        );
      } else {
        await query(
          `UPDATE vipstock SET qty = ? WHERE VIP = ? AND goods = ? AND off != 1`,
          [diffQty, ordInfo.vip, goods]
        );
      }
      if (goodsMap[goods]) {
    ...
    const result = await query(
      `SELECT * FROM vipstock WHERE nun = ? AND vip = ? AND off != 1`,
      [ordInfo.nun, ordInfo.vip]
    );
    for (let i = 0, len = result.length; i < len; i++) {
      ...
      const exist = await query(
        `SELECT * FROM vipstock WHERE nun = '' AND vip = ? AND goods = ? AND off != 1`,
        [ordInfo.vip, item.goods]
      );
      if (exist.length !== 0) {
        await query(
          `UPDATE vipstock SET qty = ? WHERE nun = '' AND vip = ? AND goods = ? AND off != 1`,
          [Number(exist[0].qty) + Number(item.qty), ordInfo.vip, item.goods]
        );
        await query(
          `DELETE FROM vipstock WHERE nun = ? AND vip = ? AND goods = ? AND off != 1`,
          [ordInfo.nun, ordInfo.vip, item.goods]
        );
      } else {
        await query(
          `UPDATE vipstock SET nun = '' WHERE nun = ? AND vip = ? AND goods = ? AND off != 1`,
          [ordInfo.nun, ordInfo.vip, item.goods]
        );
      }
    }
    ...
    for (let key in goodsMap) {
      let result = await query(
        `SELECT * FROM goods WHERE id = ? AND off != 1`,
        [key]
      );
      if (result.length !== 0) {
        await query(`UPDATE goods SET qty = ? WHERE id = ? AND off != 1`, [
          result[0].qty - goodsMap[key].qty,
          key
        ]);
      }
    }
...
router.post("/deleteOrder", async ctx => {
  try {
    ...
    if (payMethod == 5) {
      ...
      if (vipInfo.length !== 0) {
        await query(`UPDATE vip SET balance = ? WHERE phone = ?`, [
          Number(vipInfo.balance) +
            Number(ordInfo.totalPrice) -
            Number(ordInfo.discount || 0)
        ]);
      }
    ...
    if (ordInfo.depositGoods && ordInfo.depositGoods != "") {
      ...
      for (let i = 0, len = depositGoods.length; i < len; i++) {
        ...
        let exist = await query(
          `SELECT * FROM vipstock WHERE vip = ? AND goods = ? AND off != 1`,
          [ordInfo.vip, depositGoods[i]]
        );
        ...
        if (qty <= 0) {
          ...
          await query(
            `UPDATE vipstock SET qty = ? WHERE vip = ? AND goods = ? AND off != 1`,
            [qty, ordInfo.vip, depositGoods[i]]
          );
        }
      }
    }
    ...
    if (ordInfo.stockGoods && ordInfo.stockGoods != "") {
      ...
      for (let i = 0, len = stockGoods.length; i < len; i++) {
        ...
        let exist = await query(
          `SELECT * FROM vipstock WHERE vip = ? AND goods = ? AND off != 1`,
          [ordInfo.vip, stockGoods[i]]
        );
        if (exist.length === 0) {
          await query(
            `INSERT INTO vipstock (nun, vip, goods, qty) VALUES (?, ?, ?, ?)`,
            ["", ordInfo.vip, stockGoods[i], stockQty[i]]
          );
        } else {
          await query(
            `UPDATE vipstock SET qty = ? WHERE vip = ? AND goods = ? AND off != 1`,
            [
              Number(exist[0].qty) + Number(stockQty[i]),
              ordInfo.vip,
              stockGoods[i]
            ]
          );
...
router.post("/updOrder", async ctx => {
  try {
    ...
    await query(
      `UPDATE roomorder SET package = ?, packageType = ?, grpSelected = ?, goods = ?, qty = ?, stockGoods = ?, stockQty = ?, depositGoods = ?, depositQty = ?, 
...
router.post("/getOrderHistory", async ctx => {
  try {
    ...
    if (data.type === "report") {
      ordInfo = await query(
        `SELECT * FROM roomorder WHERE close = 1 AND off != 1 ORDER BY startTime DESC`
      );
    } else {
      ordInfo = await query(
        `SELECT * FROM roomorder WHERE close = 1 AND off != 1 ORDER BY createTime DESC LIMIT ?, ?`,
        [(pageNo - 1) * pageSize, pageSize]
      );
    }

    let count = await query(
      `SELECT COUNT(*) as count FROM roomorder WHERE close = 1 AND off != 1`
    );
    let result = [];
    ...
    for (let i = 0, len = ordInfo.length; i < len; i++) {
      ...
      for (let i = 0, len = goodsList.length; i < len; i++) {
        let result = await query(
          `SELECT * FROM goods WHERE id = ${goodsList[i]}`
        );
        goodsMap[goodsList[i]] = result[0];
        goodsMap[goodsList[i]].qty = qtyList[i];
      }
      ...
      const depositGoodsList = item.depositGoods
        ? item.depositGoods.split(",")
        : [];
    ...
    if (data.type === "report") {
      ...
      message.packageList = await query(
        `SELECT DISTINCT uuid as uuid, name FROM package WHERE off != 1 ORDER BY createTime ASC`
      );
    } else {
      message = result;
    }
...
router.get("/handleData", async ctx => {
  try {
    ...
    for (let i = 0, len = orderList.length; i < len; i++) {
      ...
      await query(`UPDATE roomorder SET payMethod = ? WHERE nun = ?`, [
        item.payMethod,
        item.nun
      ]);
    }
